//http://meitu.xunlei.com/albumlist.html
package nondotpicset

/*
curl http://meitu.xunlei.com/albumlist.html
需求： 下载静态网站中的图集 需要迅雷会员
记录图库资源
...
func init() {
	config := &helper.AirConfig{
		Name:      "meitu.xunlei.com",
		Domain:    "meitu.xunlei.com",
		HomePage:  "http://meitu.xunlei.com/albumlist.html",
		FirstPage: "http://meitu.xunlei.com/albumlist.html",
	}

	config.DownloadRoot = fmt.Sprintf("./%s/", config.Name)

	helper.AIR_CONFIGS[config.Name] = config
	TheXunleiMeitu.Name = config.Name
	TheXunleiMeitu.Description = config.Domain

	TheXunleiMeitu.Register()
}

var TheXunleiMeitu = &Spider{
	//Name:         THE_DOMAIN,
